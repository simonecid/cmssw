...
)
{
  ...
  std::tuple<TTrigger*, TParticle*, float, int > l1tObjectGenJetPair;
  
  TTrigger* bestMatch = NULL;
  ...
  do {
    ...
    if (foundPreliminaryMatch) {
      ...
      {
        // inverse matching
        std::tuple<TTrigger, TParticle, float> l1tObjectGenJetPair2 =
        ...
        // if inverse matchin retgrun the initial particle we are good
...
)
{
  ...
  do
  {
    ...
    for (
        typename std::vector<TParticle>::const_iterator bx0Iterator = particleCollectionHandle->begin(0);
        bx0Iterator != particleCollectionHandle->end(0);
        bx0Iterator++)
    {
